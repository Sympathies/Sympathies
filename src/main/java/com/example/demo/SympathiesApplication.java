...
public class SympathiesApplication {
    ...
    public CommandLineRunner demoUser(UserRepository repository) {
        ...
        return (args) -> {
            ...
            userRepo.save(new User("shira@example.com", "白鳥裕士", "shira", 100));
            userRepo.save(new User("shuma@example.com", "清水秀馬", "shuma", 1000));
            userRepo.save(new User("inoue@example.com", "井上将斗", "inoue", 100));
            userRepo.save(new User("hirata@example.com", "平田雄也", "hirata", 100));
            userRepo.save(new User("yanagido@example.com", "柳戸新一", "yamnagido", 100));
            userRepo.save(new User("rakuten@example.com", "ラック・テン", "rakuten", 100));
